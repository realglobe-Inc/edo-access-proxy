...
称する TA の RSA 秘密鍵を、秘密鍵ディレクトリに置く。
...
|オプション名|初期値|値|
|:--|:--|:--|
|-keyDbPath|バイナリファイルのあるディレクトリの private_keys|秘密鍵ディレクトリのパス|
|-noVeri|`false`|転送先の SSL 証明書を検証しないかどうか|
|-selfId||称する TA の ID|
|-socPort|1605|待ち受けポート番号|
...
以下では、edo-access-proxy を 1605 番ポート待ち受けで起動し、転送先 URI が https://to.example.org/api/self-destruction または http://to.example.org/api/self-destruction であるとして例を挙げる。
...
認証ヘッダが追加されたリクエストを転送する。


#### 3.3.7. 2 回目の転送レスポンスの検査
